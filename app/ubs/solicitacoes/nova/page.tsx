...
export default function NewRequestPage() {
  ...
  const filtered = useMemo(
    () =>
      medications
        .filter((item) =>
          `${item.name} ${item.form} ${item.category}`
            .toLowerCase()
      ...
      context.registerTool(
        {
          ...
          description:
            'Cria e envia uma solicitação de medicamentos da UBS Dr. Fernando Couto para análise da CAF.',
          inputSchema: {
            type: 'object',
            properties: {
              items: {
          ...
          execute(input) {
            ...
            if (
              ...
              throw new Error(
                'Informe ao menos um medicamento e uma quantidade inteira positiva.',
              );
            setCart(
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        description="Selecione os medicamentos necessários e revise as quantidades antes do envio."
      />
      {feedback && (
        <output
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader className="border-b">
              <CardTitle className="text-lg">Adicionar medicamentos</CardTitle>
              <p className="text-xs text-slate-500">
                Busque pelo princípio ativo, nome ou categoria.
              </p>
            </CardHeader>
            <CardContent>
              ...
              <div className="mt-4 divide-y rounded-xl border">
                {filtered.map((medicine) => (
                  ...
                  >
                    ...
                    <Button
                      variant={
                        cart.some((item) => item.name === medicine.name)
                          ? 'secondary'
                          : 'outline'
                      }
                      size="sm"
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Label htmlFor="observation">Contexto da solicitação</Label>
              <Textarea
                id="observation"
                value={observation}
                onChange={(event) => setObservation(event.target.value)}
                className="min-h-28"
                placeholder="Informe campanhas, aumento de demanda ou outra justificativa relevante…"
              />
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="h-fit xl:sticky xl:top-24">
          <CardHeader className="border-b">
            <CardTitle className="flex items-center gap-2 text-lg">
              <ShoppingBasket className="size-5 text-[#5d87ff]" />
          ...
          <CardContent className="space-y-4">
                ...
                <div key={item.name} className="rounded-xl border p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-slate-500">
                        Sugestão: {item.suggested} unidades
                      </p>
                    </div>
                    <button
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <Input
                        ...
                        value={item.quantity}
                        onChange={(event) =>
                          setQuantity(item.name, Number(event.target.value))
                        }
                  ...
                  {item.quantity > item.suggested && (
                    <p className="mt-2 rounded-md bg-amber-50 px-2 py-1.5 text-[11px] text-amber-800">
                      Acima do estoque mínimo sugerido ({item.suggested} un.).
                    </p>
                  )}
                </div>
              ))
            ...
            <div className="space-y-2 border-t pt-4">
              <Button
                ...
                onClick={() => setFeedback('Rascunho salvo neste dispositivo.')}
              >
                Salvar rascunho
              </Button>
              <Button
